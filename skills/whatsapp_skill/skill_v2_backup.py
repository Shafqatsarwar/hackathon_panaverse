...
class WhatsAppSkill:
    ...
    async def _scan_messages(self, keywords: List[str] = None, check_archived: bool = True, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            async def parse_chats():
                ...
                # Updated to use more generic role="row" which is stable in WhatsApp Web
    ...
    def check_messages(self, keywords: List[str] = None, check_archived: bool = True, limit: int = 20) -> List[Dict[str, Any]]:
        """Check for messages matching keywords, optionally in archived"""
        if not self.enabled: return [{"error": "Disabled"}]
        return self._run_async_safe(self._scan_messages(keywords, check_archived, limit))
